...
public class RadixSort {
   ...
   @SuppressWarnings("unchecked")
    ...
    public static void radixSort(String[] arr, int maxLen){
        final int BUCKETS = 256;
        ArrayList<String>[] wordsByLength = new ArrayList[BUCKETS];
        ArrayList<String>[] buckets = new ArrayList[BUCKETS];
        ...
        int stratingIndex = arr.length;
        for(int pos = maxLen - 1; pos >=0; pos--){
            stratingIndex -= wordsByLength[pos + 1].size();
            for(int i = stratingIndex; i < arr.length; i++){
